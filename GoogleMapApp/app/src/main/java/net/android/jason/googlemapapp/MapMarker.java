...
/**
 * This class provides methods to mark and display locations on a map. And this
 * class also provides async implementation for background map update.
 */
class MapMarker extends AsyncTask<String, Void, List<LatLng>> {

    MapMarker(GoogleMap mMap, Context context) {
        map = mMap;
    ...
    Address getAddress(String address) {
        ...
        try {
            ...
            Log.e(clazz, "Failed to get the location's geo information", re);
        }
        return null;
    }
     ...
     * @param zoomIn
     */
    void moveCameraToMarkers(LatLng loc, final double zoomIn) {
     ...
     * UI thread.
     *
     * @param addresses The parameters of the task. This method actually accept
     *                  multiple addresses
     ...
     *
     * @param locations The result of the operation computed by {@link
     *                  #doInBackground}.
     * @see #onPreExecute
     * @see #doInBackground
    ...
    protected void onPostExecute(List<LatLng> locations) {
        ...
        double zoomIn = (locations.size() > 5) ? 10.0 : 15.0;
